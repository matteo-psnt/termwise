...
// A single ai.Message with tool results expands into multiple tool messages.
func toMessageParams(msgs []ai.Message) ([]openai.ChatCompletionMessageParamUnion, error) {
...
func toMessageParam(m ai.Message) ([]openai.ChatCompletionMessageParamUnion, error) {
	switch m.Role {
	case "user":
...
func toUserMessageParams(m ai.Message) []openai.ChatCompletionMessageParamUnion {
	var out []openai.ChatCompletionMessageParamUnion
	if m.Content != "" {
...
func toAssistantMessageParam(m ai.Message) (openai.ChatCompletionMessageParamUnion, error) {
...
func toToolParams(tools []ai.ToolDef) []openai.ChatCompletionToolParam {
...
func fromResponse(msg *openai.ChatCompletion) *ai.CompleteResponse {
	var content string
...
func fromChatResponse(msg *openai.ChatCompletion) (*ai.ChatResponse, error) {
...
func filterModels(provider string, models []ai.Model) []ai.Model {
	if provider != "openai" {
		return models
	}
