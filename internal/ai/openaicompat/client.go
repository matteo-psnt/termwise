...
func init() {
	for name, baseURL := range providerDefaults {
		...
		ai.Register(name, func(cfg ai.ProviderConfig) (ai.AgentProvider, error) {
			return New(name, baseURL, cfg)
		})
	}
}

// Client implements ai.AgentProvider for any OpenAI-compatible API.
type Client struct {
	sdk      openai.Client
	provider string
...
func New(provider, defaultBaseURL string, cfg ai.ProviderConfig) (*Client, error) {
...
func mapAPIErr(err *openai.Error) *ai.ProviderError {
	body := err.Error()
