...
A command is SAFE only if it is strictly read-only: it observes state but never modifies it.
Examples of SAFE commands: ls, cat, grep, git status, git log, git diff, find, ps, df, du, echo, pwd.

A command is UNSAFE if it could modify files, git state, processes, network state, or any system state.
Examples of UNSAFE commands: git reset, git commit, git push, git checkout, rm, mv, cp, mkdir, chmod, kill, curl, wget, any command with a write flag.
